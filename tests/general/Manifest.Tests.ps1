Describe "Validating the module manifest" {
	...
	Write-PSFMessage -Level Debug -Message "PesterTest: ModuleRoot - $ModuleRoot"

	$manifest = ((Get-Content "$moduleRoot\ps-menu.psd1") -join "`n") | Invoke-Expression
	[version]$moduleVersion = Get-Item "$moduleRoot\ps-menu.psm1" | Select-String -Pattern '\$script:PSModuleVersion = "(.*?)"' | ForEach-Object { $_.Matches[0].Groups[1].Value }
